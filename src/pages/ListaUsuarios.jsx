                ...
                <tbody className={`${theme === 'dark' ? 'table-dark': 'table-light'} no-wrap`}>
                    {usuarios.map((usuario, index) => (
                        <tr key={index}>
                            ...
                            <td>{usuario.email}</td>
                            <td>{usuario.rol}</td>
                            <td><a className="btn btn-success btn-sm" onClick={() => handleShowEditModal(usuario)}>Modificar</a></td>
                            <td><a className="btn btn-danger btn-sm" onClick={() => handleShowDeleteModal(usuario)}>Eliminar</a></td>
                        </tr>
                    ))}
                </tbody>
            </table>
            ...
            {selectedUser && (
                <div className={`modal fade ${showEditModal ? 'show' : ''}`} style={{ display: showEditModal ? 'block' : 'none' }} tabIndex="-1" role="dialog">
                    <div className="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
                        <div className="modal-content">
                            <div className="modal-header">
                                <h5 className="modal-title text-black">Modificar Usuario</h5>
                                <button type="button" className="btn-close" onClick={() => setShowEditModal(false)} aria-label="Close"></button>
                            </div>
                            <div className="modal-body text-black">
                                <form onSubmit={handleEditSubmit}>
                                    <div className="form-floating mb-3">
                                        <input type="text" name="dni" value={formData.dni} onChange={handleEditChange} className="form-control" id="dni" placeholder="DNI" required />
                                        <label htmlFor="dni">DNI</label>
                                    </div>
                                    <div className="form-floating mb-3">
                                        <input type="text" name="nombre" value={formData.nombre} onChange={handleEditChange} className="form-control" id="nombre" placeholder="Nombre" required />
                                        <label htmlFor="nombre">Nombre</label>
                                    </div>
                                    <div className="form-floating mb-3">
                                        <input type="text" name="apellido" value={formData.apellido} onChange={handleEditChange} className="form-control" id="apellido" placeholder="Apellido" required />
                                        <label htmlFor="apellido">Apellido</label>
                                    </div>
                                    <div className="form-floating mb-3">
                                        <input type="email" name="email" value={formData.email} onChange={handleEditChange} className="form-control" id="email" placeholder="Email" required />
                                        <label htmlFor="email">Email</label>
                                    </div>
                                    <div className="form-floating mb-3">
                                        <input type="password" name="contra" value={formData.contra} onChange={handleEditChange} className="form-control" id="email" placeholder="Email" required />
                                        <label htmlFor="email">Contraseña</label>
                                    </div>
                                    <div className="form-floating mb-3">
                                        <select name="rol" value={formData.rol} onChange={handleEditChange} className="form-select" id="rol" required>
