...
export const Home = () => {
  const { data, isLoading, error } = useSiniestros();
  ...
  const siniestros = data?.content ?? [];
  const siniestrosAnalista = siniestros.filter(
    (data) => data?.analista?.dni == user?.dni
  );
  ...
  return (
    <main className="mt-5 px-5">
      ...
      <h4 className="text-success mt-5">
        Siniestros en gestión: {siniestrosAnalista.length}
      </h4>
